...
def get_func_from_saved_model(saved_model_dir):
  saved_model_loaded = tf.saved_model.load(
      saved_model_dir, tags=[tag_constants.SERVING])#.expect_partial() # no expect partial before
...
def run_inference(graph_func,
  ...
  if mode == 'validation':
    for i, batch_images in enumerate(dataset):
      os.system('nvidia-smi --query-gpu=utilization.gpu,utilization.memory,memory.total,memory.free,memory.used,power.draw --format=csv >> gpu_logs_new_power/' + args.log_name)  
      start_time = time.time()
...
if __name__ == '__main__':
  ...
  parser.add_argument('--log_name', type=str, default=None, help='Name of GPU log to write to..')
  ...
  os.system('python get_gpu_stats.py gpu_logs_new_power/' + args.log_name)
